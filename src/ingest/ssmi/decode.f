...
C
C $Id: decode.f,v 1.3 1993-06-21 21:53:34 granger Exp $
C 
...
C
     ...
     1 127,128/
      DATA JNDEX/
     1 5,1,9,13,9,17,21,17,25,29,25,33,37,33,41,45,41,49,53,49,57,61,57,
     1 65,69,65,73,77,73,81,85,81,89,93,89,97,101,97,105,109,105,113,
     1 117,113,121,3,1,5,7,5,9,11,9,13,15,13,17,19,17,21,23,21,25,27,25,
     1 29,31,29,33,35,33,37,39,37,41,43,41,45,47,45,49,51,49,53,55,53,
     1 57,59,57,61,63,61,65,67,65,69,71,69,73,75,73,77,79,77,81,83,81,
     1 85,87,85, 89, 91, 89, 93, 95, 93, 97, 99, 97,101,
     1 103,101,105,107,105,109,111,109,113,115,113,117,119,117,121,
     1 125,123,127,2,1,3,4,3,5,6,5,7,8,7,9,10,9,11,12,11,13,14,13,15,16,
     1 15,17,18,17,19,20,19,21,22,21,23,24,23,25,26,25,27,28,27,29,
...
C
      ...
      INTEGER*4 IWORK4,ITAV,IRES,ITOIL1,ITOIL2,ITOIL3,ITOIL4
C
C     SPECIFY COMMON /INDATA/
C
      INTEGER*2 IBUF
...
C     PRINT *,'2^16*',IBUF(N+377),'+2^8*',IBUF(N+378),'+',IBUF(N+379),
C    $' = ',IWORK4
      ITAV=INT(IWORK4/4096)
C     PRINT *,'ITA19V = ',ITAV
      ITALO(1)=ITAV
      ITALO(2)=IWORK4-4096*ITAV
C     PRINT *,'ITA19H = ',ITALO(2)
C
...
C
      ...
      ENDIF
C     PRINT *,'TALO(19V) = ',TALO(1,ICEL)
C     PRINT *,'TALO(19H) = ',TALO(2,ICEL)
   20 CONTINUE
C
